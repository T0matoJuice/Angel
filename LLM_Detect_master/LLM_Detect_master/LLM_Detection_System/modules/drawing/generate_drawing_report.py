...
def create_detail_table(data: dict):
    ...
    # 12个检测项目的名称（按固定顺序）
    point_names = [
        '关键尺寸识别:检查关键尺寸是否标注"△"符号',
        '人员参数检查:检查设计、审核、批准等人员信息是否填写',
        '未注公差表检查:检查右上角或右下角是否存在未注公差表',
        '安吉尔LOGO检查:检查右下角标题栏是否存在企业LOGO,LOGO是否正确',
        '中文名称检查:检查标题栏的名称栏是否填写正确',
        '材料信息检查:检查标题栏的材料栏是否填写材料名称',
        '重量信息检查:检查标题栏的重量栏是否填写重量信息',
        '尺寸公差检测:检测公差值是否低于未注公差表限值',
        '公差精确度检测:检测公差精确度是否与尺寸精确度一致',
        '技术要求检测:检查技术要求内容是否合格',
        '图号检查:检查图号栏是否填写且格式正确',
        '重量信息检查:检查标题栏的重量栏是否填写重量信息'
    ]
