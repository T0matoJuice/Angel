...
# 检测1: prompt_1.txt（爆炸图或水路图跳过模型检测）
        ...
        if drawing_type in ["爆炸图", "水路图", "水路图文件"]:
...
# 检测2: 根据图纸类型选择prompt文件
        print(f"\n🔍 [2/12] 进行人员参数检查...")
        if drawing_type in ["钣金件", "塑胶件", "电器件", "总成图", "总装图", "金属件"]:
...
# 检测3: prompt_3.txt（爆炸图或水路图跳过模型检测）
        ...
        if drawing_type in ["爆炸图", "水路图", "水路图文件", "总装图", "总成图"]:
            cleaned_result = '''**第3条检测结果：**
- 检测项目：未注公差表检查
- 检测结果：符合
- 发现内容：无
- 位置描述：无
- 符合/不符合原因：图纸类型为水路图、爆炸图、总装图，该类图纸无公差
- 修改建议：无'''
            all_result += f"{cleaned_result.strip()}\n\n"
            if re.search(r'- 检测结果[：:]\s*不符合', cleaned_result):
                non_conforming_count += 1
...
# 检测6: prompt_6.txt（材料信息检查，需后处理结果）
 ...
 # 检测7: prompt_7.txt（重量信息检查，需后处理结果）
        ...
        if drawing_type not in ["塑胶件", "钣金件", "金属件", "爆炸图", "水路图", "水路图文件"]:
            cleaned_result = '''**第7条检测结果：**
- 检测项目：重量信息检查
- 检测结果：符合
- 发现内容：无
- 位置描述：无
- 符合/不符合原因：图纸类型为电器件或总装图，该类图纸直接判定为符合
- 修改建议：无'''
            all_result += f"{cleaned_result.strip()}\n\n"
            if re.search(r'- 检测结果[：:]\s*不符合', cleaned_result):
                non_conforming_count += 1
            ...
            if prompt_file.exists():
                ...
                if weight_content_match:
                    ...
                    # 爆炸图/水路图：重量填为"/"才算符合
                    elif drawing_type in ["爆炸图", "水路图", "水路图文件"]:
                        if weight_content == "/":
                            # 改为符合
                            cleaned_result = re.sub(r'(- 检测结果[：:]\s*)不符合', r'\1符合', cleaned_result)
                            cleaned_result = re.sub(r'(- 符合/不符合原因[：:]\s*)[^\n]+', r'\1图纸中重量信息为"/"',
