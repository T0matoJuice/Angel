...
class Processor:
    ...
    def apply_quality_rules(self, messages: list, test_excel: str) -> tuple:
        ...
        try:
            ...
            # 构建AI判断提示词
            quality_prompt = f"""系统角色与任务：
你是具备家电售后质量分析专家角色的AI系统，需依据维修工单完整信息，按照多层级决策逻辑判断工单性质（质量工单/非质量工单），并输出可追溯的判定依据。决策需严格遵循"保内优先、换件分类、描述加权、特殊规则补充"的原则，确保结果可解释、高精度。

核心判断逻辑框架
第一层：保内保外筛查
1.  若工单为「保外」（需验证安装日期格式为YYYY/MM/DD的有效性），默认判定为非质量工单
2.  保外特殊规则：若故障部位包含"电磁阀"、"镇流器"、"紫外灯"等电子易损元件关键词，则判定为质量工单
3.  若工单为「保内」，进入第二层判断

第二层：判断故障类别
1.  以下类别默认判定为非质量工单，但需结合换件信息、故障描述和处理方案最终确认：
    - 滤芯类故障
    - 使用环境类故障
    - 外观结构类故障
    - 产品鉴定类工单
2.  不属于上述类别的工单，进入第三层判断

第三层：换件（旧件和新件）情况分流
1.  有换件（「旧件」非空且「新件」非空）：进入第四层配件类型分析
2.  无换件（「旧件」为空 或 「新件」仅含清洗/调整类操作）：进入第五层综合加权分析

第四层：换件类型判定规则
1.  增加/更换机器主体外配件（如压力桶、外接活性炭、阻垢滤芯、排水组件）：非质量工单，判定结束
2.  维修方案为空或不明确：非质量工单，判定结束
3.  故障部位包含"电磁阀"、"镇流器"、"紫外灯"等电子易损元件：质量工单，判定结束
4.  核心零配件更换判定：
    a. 滤芯类（新件名称含"活性炭滤芯"、"后置活性炭滤芯"、"复合滤芯"、"复合磷酸盐滤芯"、"反渗透膜滤芯成品"）：
       - PP复合滤芯：安装日期至报修日期 ≤30天 → 质量工单
       - 反渗透膜滤芯(及复合)：安装日期至报修日期 ≤90天 → 质量工单
       - 后置含炭滤芯：安装日期至报修日期 ≤30天 → 质量工单
       - 特殊场景：工单描述含"漏碳""异味"且使用时长≤30天 → 质量工单
    b. 滤网/前置过滤器（故障部位含"滤网""前置过滤"）：
       - 现场诊断描述含"堵塞""出水小""卡住" → 非质量工单（判定为水源问题）
5.  加装类配件（无旧件、有新件）：
    - 新件名称匹配"活性炭"、"排水"、"压力桶" → 非质量工单（判定为增值服务）

第五层：无换件综合加权判定（总分100分，≥60分判定为质量工单）
1.  故障描述与处理方案交叉验证（权重60%，总分60分）：
    a. 处理方案含简单操作（调节、紧固、重置、重启、排空、更换管子/三通/接头/垫片、调试等）→ 0分
    b. 处理方案含"机器正常""检查正常""无更换"等描述 → 0分
    c. 内部部件调整（泵松动调整、管接头加固、电路板焊接等）→ 42分
    d. 外部操作（重新插管、调整外部管路、安装复位等非侵入性操作）→ 18分
2.  报修内容分析（权重40%，总分40分）：
    a. 清晰描述：报修内容明确指向具体部件故障（如"泵异响""管破裂"），且维修方案包含指向性操作 → 28分
    b. 模糊描述：仅提及"漏水""不出水""不工作"等现象，且维修方案无明确部件指向 → 12分
3.  维修时间修正项（±20分）：
    - 安装日期至维修日期≤14天，且无明确故障描述 → 扣20分（最终得分=原得分×50%）

第六层：特殊场景覆盖
1.  电商渠道产品：若关联鉴定单且鉴定结果为"产品缺陷" → 质量工单，否则按常规逻辑判定
2.  视频证据补充：若维修时间字段关联视频链接，且音频分析含"部件更换声""异常噪音" → 额外加10分（仅用于边界案例）

决策约束与校准：
1.  日期计算统一转换为天数（忽略时分），避免跨月误差
2.  模糊描述类工单（如"不出水"）必须交叉验证「故障部位名称」和「维修方案」，禁止单字段判定
3.  「产品类别组」为"净水机""净饮机"的工单，滤芯使用时长判定标准严格优先于其他规则

请根据上述判断规则，对下面的测试数据进行工单性质判断。

测试数据（CSV格式）：
{test_content}

输出格式要求：
- 必须包含所有3个字段：工单单号,工单性质,判定依据
