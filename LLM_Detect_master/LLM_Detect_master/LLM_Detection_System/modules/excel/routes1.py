...
def quality_data_upload():
    ...
    try:
        ...
        # 3. 获取账号和文件名
        account = data.get('account', 'api_user').strip()
        custom_filename = data.get('filename', '').strip()
        
        # 生成唯一的批次ID
        timestamp_str = datetime.now().strftime('%Y%m%d_%H%M%S')
        if custom_filename:
            batch_id = f"{custom_filename}_{timestamp_str}"
        else:
            batch_id = f"api_upload_{timestamp_str}"
        ...
        for index, workorder in enumerate(workorders):
            try:
                ...
                workorder_no = str(workorder.get('工单单号', '')).strip()
                if not workorder_no or workorder_no == 'nan':
                    error_list.append({
                        'index': index,
                        'error': '工单单号不能为空'
                    })
